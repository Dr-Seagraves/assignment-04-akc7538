...
def plot_scatter_with_regression(
    ...
    ax.scatter(plot_df[x_var], plot_df["ret"], alpha=0.5, label="Observations")
    ...
    ax.plot(x_grid, y_grid, color="black", linewidth=2, label="OLS fit")

    y_low, y_high = np.percentile(plot_df["ret"], [2, 98])
    ax.set_xlim(x_low, x_high)
    ax.set_ylim(y_low, y_high)

    ax.set_title(f"{title} (R^2 = {model.rsquared:.3f})")
...
def print_key_results(model, x_var: str) -> None:
    ...
    print(
        "Intercept (b0): "
        f"{intercept:.6f} | SE: {se_intercept:.6f} | t: {t_intercept:.3f} | p: {p_intercept:.4f}"
    )
    print(
        "Slope (b1):     "
        f"{slope:.6f} | SE: {se_slope:.6f} | t: {t_slope:.3f} | p: {p_slope:.4f}"
    )
    print(
        f"R^2: {model.rsquared:.4f} | Adj R^2: {model.rsquared_adj:.4f} | N: {int(model.nobs)}"
    )
